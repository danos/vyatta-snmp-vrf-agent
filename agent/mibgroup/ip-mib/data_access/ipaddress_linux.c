...
void
netsnmp_arch_interface_get_vrf(const char *name, char *vrf)
{
        FILE *fin;
        char *path;
        *vrf = '\0';
        ...
        if (asprintf(&path, "/sys/class/net/%s/rdid", name) > 0) {
            fin = fopen(path, "r");
...
#else
        /* master is a symlink to the interface's vrf */
        if (asprintf(&path, "/sys/class/net/%s/master", name) > 0) {
            char buf[200] = {0};
            if (readlink(path, buf, 200) > 0) {
...
#define PROCFILE "/proc/net/if_inet6"
    ...
    while (fgets(line, sizeof(line), in)) {
        ...
        netsnmp_arch_interface_get_vrf(if_name, entry->ia_vrf);
