                                                  ...
                                                  int idx_offset)
{
...
#ifdef HAVE_ROUTING_DOMAIN
    (void) setsockopt(sd, SOL_SOCKET, SO_RTDOMAIN, &rd_any, sizeof(rd_any));
#endif
    ...
    for(i=0; i < interfaces; ++i, ++ifrp) {
        ...
        netsnmp_arch_interface_get_vrf(extras->name, entry->ia_vrf);
        ...
        if(addr_info.bcastflg) {
           ...
           strcpy(bcastentry->ia_vrf, entry->ia_vrf);
