 ...
 */
int _access_ipaddress_entry_compare_addr(const void *lhs, const void *rhs);
                                      ...
                                      netsnmp_ipaddress_entry *rhs)
{
    ...
    if (strcmp(lhs->ia_vrf, rhs->ia_vrf) != 0) {
        ++changed;
        strcpy(lhs->ia_vrf, rhs->ia_vrf);
    }
...
int _access_ipaddress_entry_compare_addr(const void *lhs, const void *rhs)
{
    ...
    if (lh->ia_address_len < rh->ia_address_len)
        ...
        return 1;
    else if (strcmp(lh->ia_vrf, rh->ia_vrf) != 0)
        return strcmp(lh->ia_vrf, rh->ia_vrf);
