...
static int
_insert_filter(netsnmp_container *container, const void *data)
{
    char *container_name = container->container_name;
    const char *vrf = netsnmp_context_find_vrf(container_name);
    const ipAddressTable_rowreq_ctx *rowreq_ctx = data;

    return vrf_compare(vrf, rowreq_ctx->data->ia_vrf);
}
                                     ...
                                     u_long flags, char *contextName)
{
    ...
    if (asprintf(&container_name, "ipAddressTable_%s:table_container",
                                  contextName ? contextName : "") < 0) {
        return;
    }

    container = netsnmp_container_find(container_name);
    free(container_name);
    if (NULL == container) {
        snmp_log(LOG_ERR,
    ...
    container->insert_filter = _insert_filter;
    container->container_name = strdup(contextName ? contextName : "");

    netsnmp_container_add_index(ipAddressTable_if_ctx.container,
                                container );

    /*
     * Since this MIB is autoloaded, we need to clear
     * any existing data in the container and cache
     * so that the secondary containers get filled.
     */
    _container_free(ipAddressTable_if_ctx.container);
    if (NULL != ipAddressTable_if_ctx.cache) {
        netsnmp_cache *cache = ipAddressTable_if_ctx.cache;
