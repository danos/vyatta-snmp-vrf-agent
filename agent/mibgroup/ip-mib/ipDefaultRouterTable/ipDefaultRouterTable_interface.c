...
_insert_filter(netsnmp_container *container, const void *data)
{
    ...
    char *vrf = netsnmp_context_find_vrf(container_name);
    const ipDefaultRouterTable_rowreq_ctx *rowreq_ctx = data;

    return match_vrf_to_id(vrf, rowreq_ctx->data->dr_vrf_table_id);
}
     ...
     char *contextName)
{
    ...
    if (asprintf(&container_name, "ipDefaultRouterTable_%s:table_container",
    ...
    netsnmp_container_add_index(ipDefaultRouterTable_if_ctx.container,
                                container );
