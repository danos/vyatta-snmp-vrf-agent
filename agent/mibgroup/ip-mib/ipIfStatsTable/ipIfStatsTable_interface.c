                                     ...
                                     u_long flags, char *contextName)
{
     ...
     */
    {
        ...
        reginfo = netsnmp_create_handler_registration(
                    "ipIfStatsTableLastChanged", NULL, lc_oid,
                    OID_LENGTH(lc_oid), HANDLER_CAN_RONLY);
        if (contextName)
            reginfo->contextName = strdup(contextName);

        netsnmp_register_watched_scalar2(reginfo,
                netsnmp_create_watcher_info((void *)
                    &ipIfStatsTable_if_ctx.last_changed,
                    sizeof(u_long), ASN_TIMETICKS, WATCHER_FIXED_SIZE));
    }
                           ...
                           netsnmp_variable_list * var, int column)
{
    ...
    case COLUMN_IPIFSTATSHCINRECEIVES:
        var->val_len = sizeof(__netsnmp_u64__);
        var->type = ASN_COUNTER64;
        rc = ipIfStatsHCInReceives_get(rowreq_ctx,
                                       (__netsnmp_u64__ *) var->val.string);
        break;
    ...
    case COLUMN_IPIFSTATSHCINOCTETS:
        var->val_len = sizeof(__netsnmp_u64__);
        var->type = ASN_COUNTER64;
        rc = ipIfStatsHCInOctets_get(rowreq_ctx, (__netsnmp_u64__ *) var->val.string);
        break;
    ...
    case COLUMN_IPIFSTATSHCOUTOCTETS:
        var->val_len = sizeof(__netsnmp_u64__);
        var->type = ASN_COUNTER64;
        rc = ipIfStatsHCOutOctets_get(rowreq_ctx, (__netsnmp_u64__ *) var->val.string);
        break;
