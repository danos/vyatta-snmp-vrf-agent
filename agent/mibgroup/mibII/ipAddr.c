...
static int      Address_Scan_Next(short *, struct ifnet *, char *);
#else
...
#if defined(linux)
        if (Address_Scan_Next(&interface, &ifnet, contextName) == 0)
            break;
#else
...
#ifdef HAVE_ROUTING_DOMAIN
    (void) setsockopt(nlsk, SOL_SOCKET, SO_RTDOMAIN, &rd_any, sizeof(rd_any));
#endif
...
static void
Enumerate_Link(void *data, int len)
{
    struct ifaddrmsg *ifa = data;
    struct rtattr *rta;
    struct ifnet *iter;
    char vrf[IFNAMSIZ + 1] = "default";

    rta = IFLA_RTA(ifa);
    while (RTA_OK(rta, len)) {
        switch (rta->rta_type) {
#ifdef HAVE_ROUTING_DOMAIN
        case IFLA_RTG_DOMAIN:
            sprintf(vrf, "%d", *(int *)RTA_DATA(rta));
            break;
        }
#else
        case IFLA_MASTER:
            if_indextoname(*(int *)RTA_DATA(rta), vrf);
            break;
        }
#endif
        rta = RTA_NEXT(rta, len);
    ...
    for (iter = ifr; iter; iter = iter->if_next) {
        if (iter->if_index == ifa->ifa_index)
            strcpy(iter->if_vrf, vrf);
    }
}
...
Address_Scan_Init(void)
{
    ...
    Netlink_Scan(RTM_GETLINK, AF_INET, Enumerate_Link);
}

static int
Address_Scan_Next(short *Index, struct ifnet *Retifnet, char *contextName)
{
    const char *vrf = netsnmp_context_find_vrf(contextName);
    struct ifnet *ifp = ifr;
    struct ifnet *ifp_prev = NULL;

    while (ifp) {
        if (vrf_compare(vrf, ifp->if_vrf))
            break;
        ifp_prev = ifp;
        ifp = ifp->if_next;
    }
    ...
    if (ifp) {
        if (Retifnet) {
            Retifnet->ifu_broadaddr = ifp->ifu_broadaddr;
            Retifnet->if_addr = ifp->if_addr;
            Retifnet->ia_subnetmask = ifp->ia_subnetmask;
        }
        if (Index)
            *Index = ifp->if_index;
        if (!ifp_prev)
