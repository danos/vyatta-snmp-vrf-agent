...
void
register_ipv6_context(char *contextName)
{
    register_mib_context("mibII/ipv6",
                         (struct variable *) ipv6_variables,
                         sizeof(struct variable3),
                         sizeof(ipv6_variables)/sizeof(struct variable3),
                         ipv6_variables_oid,
                         sizeof(ipv6_variables_oid)/sizeof(oid),
                 ...
                 int from, int to)
{
    ...
    memcpy((char *) newname, (char *) vp->name,
           (int) vp->namelen * sizeof(oid));
    for (i = from; i <= to; i++) {
        char *if_name = if_getname(i);
        char if_vrf[200] = "default";

        if (if_name)
            netsnmp_arch_interface_get_vrf(if_name, if_vrf);
        ...
        result =
            snmp_oid_compare(name, *length, newname,
                             (int) vp->namelen + 1);
        if (((exact && result == 0) || (!exact && result < 0))
            && if_name && ((vrf && !strcmp(vrf, if_vrf) ||
            ((!vrf || strlen(vrf) == 0) && !strcmp(if_vrf, "default")))))
            break;
    }
    if (to < i)
        return (MATCH_FAILED);
              ...
              char * contextName)
{
    ...
    if (header_ipv6_scan
        (vp, name, length, exact, var_len, write_method, contextName, 1, max)
        == MATCH_FAILED) {
        return NULL;
    }
    interface = name[*length - 1];
